...
public class NaiveRangedListener extends ListenerAdapter {
	...
	public NaiveRangedListener(Logger log, SimpleCert cert,
			List<Object> ranges, int newDepth) {
		this.log = log;
		this.cert = cert;
		this.ranges = ranges;
	...
	public void searchStarted(Search search) {
		...
		assert cert != null;
		assert ranges != null;
		assert startPath != null || endPath != null;
		...
		if (executedInstruction instanceof IfInstruction
			...
			if (nextInstruction != executedInstruction
				...
				if (cg instanceof PCChoiceGenerator) {
					...
					if (!inRange) {
						if (!PathConditionOps.pathSatisfiableForStart(pc, theSymValues,
					...
					if (inRange && curDepth == newDepth) {
						if (!enterLastPath
								&& PathConditionOps.pathSatisfiableForEnd(pc,
		...
		if (executedInstruction instanceof ReturnInstruction) {
			if (cert != null && cert.getName().equals(mi.getFullName())) {
				...
				if (cg instanceof PCChoiceGenerator) {
					...
					if (PathConditionOps.pathSatisfiableForEnd(pc, theSymValues,
							(List<Object>) endPath)) {
						inRange = false;
						enterLastPath = true;
					}
