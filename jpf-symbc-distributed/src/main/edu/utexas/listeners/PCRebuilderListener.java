...
import edu.utexas.util.MySATService;
...
public class PCRebuilderListener extends ListenerAdapter {

	Logger log;
	SimpleCert cert;
	List<Object> ranges = new ArrayList<>();
	List<Object> allRanges = new ArrayList<>();
	int oldDepth;
	int newDepth;
	int curDepth = 0;
	...
	boolean isSingleRange = false;

	Iterator<Object> rangeIter;

	List<Object> finalPaths = new ArrayList<>();

	public PCRebuilderListener(Logger log, SimpleCert cert, int oldDepth,
			int newDepth) {
		this.log = log;
		this.cert = cert;
		this.allRanges = cert.getPathsGroups();
		assert !allRanges.isEmpty();
		rangeIter = allRanges.iterator();
				...
				if (cg instanceof PCChoiceGenerator) {
					...
					if (!isSingleRange) {
						if (curDepth <= oldDepth && !inRange) {
							if (!PathConditionOps.pathSatisfiable(pc,
									theSymValues, (List<Object>) startPath)) {
								vm.getSearch().setIgnoredState(true);
								storeUnsatPCInfo(pc, conf);
								return;
						...
						if (curDepth <= oldDepth) {
							if (!PathConditionOps.pathSatisfiable(pc,
									theSymValues, (List<Object>) singlePath)) {
								vm.getSearch().setIgnoredState(true);
								storeUnsatPCInfo(pc, conf);
								return;
							} else {
...
//					}
	...
	private void getNextRange() {
		if (rangeIter.hasNext()) {
			ranges = (List<Object>) rangeIter.next();
			if (ranges.size() == 2) {
	...
	private void storeUnsatPCInfo(PathCondition pc, Config conf) {
		if (conf.getBoolean("symbolic.green")) {
			MySATService.isSat = false;
			pc.solveGreen();
		}
	}

	private void storeSatPCInfo(PathCondition pc, Config conf) {
		if (conf.getBoolean("symbolic.green")) {
			MySATService.isSat = true;
			pc.solveGreen();
		}
	}
